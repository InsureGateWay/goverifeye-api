...
export class VerificationCodeEntity extends BaseEntity {
  ...
  @Column({ type: 'varchar', length: 32 }) code!: string; @Column({type:'varchar',length:16,nullable:true}) codeFormatVersion?:string|null; @Column({type:'varchar',length:16,nullable:true}) keyVersion?:string|null;
  @Column({type:'char',length:4,nullable:true}) @Index() namespace?:string|null; @Column({type:'bigint',nullable:true}) internalSerial?:string|null; @Column({type:'char',length:7,nullable:true}) @Index() publicToken?:string|null; @Column({type:'char',length:1,nullable:true}) luhnDigit?:string|null; @Column({type:'char',length:4,nullable:true}) antiFabTag?:string|null; @Column('uuid',{nullable:true}) allocationId?:string|null; @Column('uuid',{nullable:true}) productBatchId?:string|null; @Column('uuid',{nullable:true}) unitId?:string|null; @Column({ type: 'varchar', length: 64, nullable:true, select:false }) activationCodeHash?: string|null;
...
@Entity('scan_sessions') export class ScanSessionEntity extends BaseEntity { @Column({unique:true}) scannerHash!:string; @Column() nonceHash!:string; @Column({type:'timestamptz'}) nonceExpiresAt!:Date; @Column({type:'timestamptz'}) expiresAt!:Date; @Column({type:'timestamptz'}) lastSeenAt!:Date; @Column({nullable:true}) ipHash?:string; @Column({nullable:true}) userAgentHash?:string; }
