...
export class CodesService {
  ...
  async getBatch(organizationId: string, id: string) {
    ...
    const [product,user]=await Promise.all([this.dataSource.getRepository(ProductEntity).findOneBy({id:batch.productId,organizationId}),this.dataSource.getRepository(UserEntity).findOneBy({id:batch.generatedBy,organizationId})]);
    return {...batch,productName:product?.name,productImageUrl:product?.imageUrl,generatedByName:[user?.firstName,user?.lastName].filter(Boolean).join(' ')||user?.email||'Vendor user',generatedByImageUrl:user?.profileImageUrl,totalCost:this.batchCost(batch)};
  }
  ...
  private batchCost(batch: Pick<CodeBatchEntity,'labelType'|'quantity'>){return Number(((batch.labelType==='pair'?14.25:7.5)*batch.quantity).toFixed(2))}
}
