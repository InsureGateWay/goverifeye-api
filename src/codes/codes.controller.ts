import { Body, Controller, Get, Headers, HttpCode, Param, Post, Req } from '@nestjs/common'; import type { Request } from 'express';
...
export class CodesController {
  ...
  @Post() generate(@CurrentUser() user: RequestContext,@Headers('idempotency-key')key:string|undefined,@Body() dto: GenerateBatchDto) { return this.codes.generateBatch(user.organizationId, user.userId, dto,key); }
  ...
  @Public() @Throttle({default:{limit:30,ttl:60000}}) @Post('codes/verify') @HttpCode(200) verify(@Body() dto: VerifyProductCodeDto,@Req()request:Request) { return this.codes.verify(dto.verificationCode,{ip:request.ip,userAgent:request.get('user-agent'),location:dto.location}); }
}
