...
export class GovernanceService {
    ...
    return this.db.transaction(async (manager) => {
      ...
      await auditRepo.save(auditRepo.create({
        ...
        metadata: {
          ...
          reference,
          details: `Submitted ${saved.category} change request`,
  ...
  async auditCsv(q:ExportQueryDto,platform=false,organizationId?:string){
    const qb=this.db.getRepository(AuditLogEntity).createQueryBuilder('audit').leftJoin(UserEntity,'actor','actor.id = audit.actorId AND actor.organizationId = audit.organizationId').addSelect(['actor.firstName','actor.lastName','actor.email']);
    if(!platform){if(!organizationId)throw new DomainError('Organization context is required','ORGANIZATION_REQUIRED',400);qb.where('audit.organizationId = :organizationId',{organizationId});}
    if(q.actorId)qb.andWhere('audit.actorId = :actorId',{actorId:q.actorId});
    if(q.actor)qb.andWhere(`(LOWER(CONCAT(COALESCE(actor.firstName,''), ' ', COALESCE(actor.lastName,''))) LIKE :actor OR LOWER(COALESCE(actor.email,'')) LIKE :actor)`,{actor:`%${q.actor.toLowerCase()}%`});
    if(q.action)qb.andWhere('audit.action = :action',{action:q.action});
    if(q.resourceType)qb.andWhere('audit.resourceType = :resourceType',{resourceType:q.resourceType});
    if(q.status)qb.andWhere('audit.status = :status',{status:q.status});
    if(q.from)qb.andWhere('audit.createdAt >= :from',{from:auditDateBoundary(q.from,'from')});
    if(q.to)qb.andWhere('audit.createdAt <= :to',{to:auditDateBoundary(q.to,'to')});
    if(q.search)qb.andWhere(`(LOWER(audit.action) LIKE :search OR LOWER(audit.resourceType) LIKE :search OR LOWER(COALESCE(audit.resourceId,'')) LIKE :search OR LOWER(COALESCE(actor.firstName,'')) LIKE :search OR LOWER(COALESCE(actor.lastName,'')) LIKE :search OR LOWER(COALESCE(actor.email,'')) LIKE :search)`,{search:`%${q.search.toLowerCase()}%`});
    const {entities,raw}=await qb.orderBy('audit.createdAt','DESC').take(q.limit).getRawAndEntities();
    if(platform)return csv(['timestamp','actor','organization','action','resourceType','resourceId','status','correlationId'],entities.map(r=>[r.createdAt,r.actorId,r.organizationId,r.action,r.resourceType,r.resourceId,r.status,r.correlationId]));
    return csv(['timestamp','actor','action','module','target','outcome','details'],entities.map((row,index)=>{
      const actor=[raw[index]?.actor_firstName,raw[index]?.actor_lastName].filter(Boolean).join(' ')||raw[index]?.actor_email||'User';
      const meta=mapVendorAuditRowMetadata(row);
      return [row.createdAt,actor,row.action,row.resourceType,row.resourceId,row.status,meta.details];
    }));
  }
