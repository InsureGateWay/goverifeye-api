...
export class GovernanceService {
  ...
  async listExceptions(q: AuditExceptionQueryDto) { const r=this.db.getRepository(AuditExceptionEntity); const where:any={...(q.severity?{severity:q.severity}:{}),...(q.status?{status:q.status}:{}),...(q.search?{title:ILike(`%${q.search}%`)}:{})}; const [rows,total]=await r.findAndCount({where,order:toOrder(q.sortBy,q.sortDirection,['createdAt','updatedAt','severity','status'] as const,'createdAt'),skip:(q.page-1)*q.pageSize,take:q.pageSize});return {items:rows.map(x=>({id:x.id,exception:x.title,severity:x.severity,age:age(x.createdAt),status:x.status,closure:x.resolutionComment?{comment:x.resolutionComment,evidenceFileName:x.evidence[0]?.fileName}:undefined})),total,page:q.page,pageSize:q.pageSize,totalPages:Math.ceil(total/q.pageSize)}; }
  async createException(u:RequestContext,dto:CreateAuditExceptionDto){const r=this.db.getRepository(AuditExceptionEntity);return r.save(r.create(this.audit(u,{...dto,status:'open'})));}
