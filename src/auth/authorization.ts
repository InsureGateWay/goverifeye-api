...
@Injectable() export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext) {
    ...
    const isStaffActivationWrite=/\/code-batches\/(?:open-market\/lookup|open-market\/[^/]+\/(?:link|verify)|[^/]+\/activate)\/?$/.test(request.path);
    const isPersonalVendorStaffWrite=isStaffActivationWrite||/\/(?:auth|notifications|settings\/(?:profile|password|account))(?:\/|$)/.test(request.path);
