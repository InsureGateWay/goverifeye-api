...
export class AddressDto { @ApiProperty({example:'12 Verification Avenue'}) @IsString() line1!: string; @ApiPropertyOptional({example:'Suite 4'}) @IsOptional() @IsString() line2?: string; @ApiProperty({example:'Lagos'}) @IsString() city!: string; @ApiProperty({example:'Lagos'}) @IsString() state!: string; @ApiPropertyOptional({example:'Ikeja'}) @IsOptional() @IsString() lga?:string; @ApiProperty({example:'Nigeria'}) @IsString() country!: string; @ApiProperty({example:'100001'}) @IsString() postalCode!: string; }
export class AdministratorDto { @ApiProperty({example:'Ada'}) @IsString() firstName!: string; @ApiProperty({example:'Okafor'}) @IsString() lastName!: string; @ApiPropertyOptional({example:'Compliance Manager'}) @IsOptional() @IsString() jobTitle?:string; @ApiProperty({example:'ada@example.com',format:'email'}) @IsEmail() email!: string; @ApiProperty({example:'+2348012345678'}) @IsString() phone!: string; }
