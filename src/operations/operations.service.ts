...
@Injectable() export class OperationsService {
  ...
  async auditSummary(organizationId:string,q:AuditSummaryQueryDto){
    ...
    const totalActivities=await this.db.getRepository(AuditLogEntity).countBy({organizationId}),actorCounts=new Map<string,number>();for(const row of rows)if(row.actorId)actorCounts.set(row.actorId,(actorCounts.get(row.actorId)??0)+1);
