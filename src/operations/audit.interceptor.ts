...
export class AuditInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const path=(request.originalUrl??request.url).split('?')[0]??'',parts=path.split('/').filter(Boolean),resourceType=this.resourceType(parts),resourceId=this.resourceId(request.params),hasExplicitSuccessAudit=parts.includes('change-requests');
    ...
    const forwarded=request.headers['x-forwarded-for']?.split(',')[0]?.trim(),location=[request.headers['cf-ipcity'],request.headers['cf-region'],request.headers['cf-ipcountry']].filter(Boolean).join(', '),authority=request.user.role.split('_').map(x=>x[0]?.toUpperCase()+x.slice(1)).join(' ');
    ...
    return next.handle().pipe(tap(()=>{if(!hasExplicitSuccessAudit)write('success')}),catchError(error=>{write('failed');return throwError(()=>error)}));
  }
