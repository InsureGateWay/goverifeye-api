...
@Module({ imports: [ConfigModule.forRoot({ isGlobal: true, cache: true, load: [appConfig, databaseConfig, codeGenerationConfig], validate: validateEnvironment }),ThrottlerModule.forRoot([{ttl:60000,limit:100}]), TypeOrmModule.forRootAsync({ inject: [ConfigService], useFactory: (config: ConfigService) => config.getOrThrow<DatabaseOptions>('database') }), AuthModule, OnboardingModule, ProductModule, CodesModule, TeamModule, ReportingModule, OperationsModule,CommerceModule], controllers: [HealthController], providers: [{ provide: APP_GUARD, useClass: ThrottlerGuard },{ provide: APP_GUARD, useClass: JwtAuthGuard }, { provide: APP_GUARD, useClass: RolesGuard },{provide:APP_FILTER,useClass:DomainErrorFilter},{provide:APP_INTERCEPTOR,useClass:AuditInterceptor}] })
