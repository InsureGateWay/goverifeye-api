...
import { invitationEmail, passwordResetCodeEmail, platformVendorDecisionEmail, platformVendorOnboardingSubmittedEmail, vendorOnboardingSubmittedEmail, vendorRejectedEmail, vendorVerifiedEmail, verificationCodeEmail } from '../operations/email-templates'
  ...
  try {
    ...
    await db.transaction(async (manager) => {
      ...
      const rawTemplates=[
        ...
        {key:'platform.vendor_onboarding_submitted',name:'Vendor onboarding ready for review',audience:'platform_admin',description:'Alerts active Super Admin users when a vendor completes onboarding.',content:platformVendorOnboardingSubmittedEmail({firstName:'{{firstName}}',companyName:'{{companyName}}',vendorContactName:'{{vendorContactName}}',vendorEmail:'{{vendorEmail}}',industry:'{{industry}}',country:'{{country}}',reviewUrl:'{{reviewUrl}}'})},
