...
export class AllowControlledVendorDeletion1726100000000 implements MigrationInterface {
  name = 'AllowControlledVendorDeletion1726100000000';
  ...
  async up(q: QueryRunner): Promise<void> {
    await q.query(`CREATE OR REPLACE FUNCTION protect_code_batch_identity() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN
      IF TG_OP='DELETE' THEN
        IF current_setting('app.allow_vendor_cascade_delete', true)='on' THEN RETURN OLD; END IF;
        RAISE EXCEPTION 'Code batches cannot be deleted or reissued';
      END IF;
